...
displayInTheMiddle(`update-public.node.mjs version ${packageJson.version}`);
...
const source = getArgumentValueOrCrash(process.argv, 'source'); // public
const target = getArgumentValueOrCrash(process.argv, 'target'); // src/app/db/public.json || src/app/db/public.ts
...
if (extension === 'json') {
   ...
   */
  const stringified = JSON.stringify(publicJson, null, 4);
   ...
   * @param 4 - represents the spaces to indent.
   */
  const stringified = JSON.stringify(publicJson, null, 2);
...
display('');
display('update-public.node.mjs script reached end', '[  DONE  ]');
